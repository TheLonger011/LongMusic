...
func (h *PlaylistHandler) GetTracks(w http.ResponseWriter, r *http.Request) {
	...
	tracks, err := h.service.GetTracks(id)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
...
func (h *PlaylistHandler) SetPublic(w http.ResponseWriter, r *http.Request) {
	idStr := chi.URLParam(r, "id")
	id, err := strconv.ParseInt(idStr, 10, 64)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
	...
	if req.IsPublic {
		err = h.service.Public(id)
	} else {
		err = h.service.Private(id)
	}
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
...
func (h *PlaylistHandler) SearchPublic(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query().Get("q")
	playlists, err := h.service.SearchPublic(q)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
